...
INSERT INTO shipyard VALUES('ISS Andromeda', 'civilian', 'passenger', 2500, 1200, NULL, 90000, 80, 0, 75.0, 3, 100, 50000, 3);

--The UPDATE query specified which data entries should be updated with new information. Be careful not to omit the WHERE restraint.
--If the WHERE is omitted all data entries will be UPDATED.

SELECT * FROM shipyard WHERE shipName = 'ISS Serenity';

UPDATE shipyard SET serviceCrew = 10 WHERE shipName = 'ISS Serenity';

SELECT * FROM shipyard WHERE shipName = 'ISS Serenity';

--UPDATE on multiple entries

SELECT * from shipyard WHERE shipType = 'civilian';

UPDATE shipyard SET marines = 10 WHERE shipType = 'civilian' AND marines = 0;
